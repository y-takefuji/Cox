...
def cox_feature_selection(data):
...
def highly_variable_selection(X, feature_names, n_features=5):
...
def spearman_correlation_selection(X, y, feature_names, n_features=5):
...
# Method 1: Cox model
cox_features, cox_summary = cox_feature_selection(data)
print(f"\nTop 5 features selected by Cox model: {cox_features}")

# Method 2: Feature Agglomeration
fa_features = feature_agglomeration(X, X.columns.tolist())
print(f"\nTop features selected by Feature Agglomeration: {fa_features}")

# Method 3: Highly Variable Features
hvgs_features = highly_variable_selection(X, X.columns.tolist())
print(f"\nTop 5 features selected by highest variance: {hvgs_features}")

# Method 4: Spearman Correlation
spearman_features, spearman_df = spearman_correlation_selection(X, y, X.columns.tolist())
print(f"\nTop 5 features selected by Spearman correlation: {spearman_features}")
