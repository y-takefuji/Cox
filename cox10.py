...
def cox_feature_selection(data, n_features=10):
...
# Method 2: Modified Feature Agglomeration - Select top features across all features
def feature_agglomeration(X, feature_names, n_clusters=10, n_features=10):
    """Feature selection using Feature Agglomeration and then selecting top features"""
    ...
    for feature in feature_names:
        feature_variances[feature] = np.var(X[feature])
    
    # Sort all features by variance
    ...
    print("\nTop features by variance across all features:")
    for feature, variance in sorted_features[:n_features]:
        print(f"{feature}: {variance:.6f}")
    
    return top_features

# Method 3: Highly Variable Gene Selection (adapted for environmental features)
def highly_variable_selection(X, feature_names, n_features=10):
...
def spearman_correlation_selection(X, y, feature_names, n_features=10):
...
fa_features = feature_agglomeration(X, X.columns.tolist(), n_clusters=10, n_features=n_features)
print(f"\nTop {n_features} features selected by Feature Agglomeration (across all features): {fa_features}")
...
all_selected_features = set(cox_features + fa_features + hvgs_features + spearman_features)
print(f"Total unique features selected across all methods: {len(all_selected_features)}")

for i, method1 in enumerate(['Cox', 'FeatureAgglo', 'HighVar', 'Spearman']):
    features1 = [cox_features, fa_features, hvgs_features, spearman_features][i]
    for j, method2 in enumerate(['Cox', 'FeatureAgglo', 'HighVar', 'Spearman']):
        if j <= i:
            continue
        features2 = [cox_features, fa_features, hvgs_features, spearman_features][j]
        overlap = set(features1).intersection(set(features2))
        print(f"Overlap between {method1} and {method2}: {len(overlap)} features - {overlap}")

# Print feature sets
print("\n--- Selected Feature Sets (Top 10) ---")
